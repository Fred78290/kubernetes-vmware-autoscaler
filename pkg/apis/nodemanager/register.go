...
const (
	...
	CRDShortName = "mn"
	FullCRDName  = CRDPlural + "." + GroupName
)

const (
	StatusManagedNodeNeedToCreated = iota
	...
	StatusManagedNodeNiceLimitReached
)
...
func StatusManagedNodeReason(code int) metav1.StatusReason {
	if code >= StatusManagedNodeNeedToCreated && code <= StatusManagedNodeNiceLimitReached {
